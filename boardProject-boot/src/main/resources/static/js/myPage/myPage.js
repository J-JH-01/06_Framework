...
// 복붙하니 문제가잇는건데, 모든페이지에 searchAddress가 있짆않음
const searchAddress = document.querySelector("#searchAddress");
if(searchAddress!=null)searchAddress.addEventListener("click", execDaumPostcode);
...
if(updateInfo != null) {
    ...
    updateInfo.addEventListener("submit", async (e) => {
        ...
        // 기존 닉네임이 저장되어있는 요소에 value값 얻어오기

        const currentNickname = document.querySelector("#currentNickname").value;
        
        // 기존 닉네임과 새로 입력된 닉네임이 다른 경우에만 중복 검사를 시행
        // -> 변경된적 없다 -> 중복 검사 진행x
        if (currentNickname !== memberNickname.value){// memberNickname이 바뀔수있는 부분
            
            //비동기 요청 (fetch() API 이용)
            // async / await 사용
            // async : 비동기 함수를 만들 때 사용하는 키워드("이 함수 내에는 오래 걸리는 작업이 있어요!")
            // await : 비동기 작업의 결과를 기다릴 떄 사용 -> !!! 반드시 async 함수 안에서만 사용가능!!!
            //          -> "이 작업이 끝날때까지 기다려주세요"
            ...
            if(count == 1 ) {
                ...
                //e.preventDefault(); // 여기서하면 제출은 진행되어버림
                return;
            }
...
const dafaultImageUrl = `${window.location.origin}/images/user.png`;
// 절대경로로 기본 이미지 URL 설정
// -> http://localhost/images/user.png << 이렇게 들어감

let statusCheck = -1  //  -1: 초기상태, 0:이미지 삭제 , 1: 새 이미지 선택 -> 일종의 플래그임
let previousImage = profileImg.src // 이전기록 이미지 URP ( 초기상태이미지의 URL 선택)
let previousFlie = null; // 이전에 선택된 파일을 객체를 저장

// 이미지 선택 시 미리보기 및 파일 크기 검사
imageInput.addEventListener("change", ()=>{
    //change 이벤트 : 기존에 있던 값과 달라지면 change 이벤트가 일어남
    
    //console.log(imageInput.files);

    // FileList (input 태그는 FileList로 저장)
    ...
    if(file){ // 파일이 선택된 경우
        if(file.size <= MAX_SIZE){ // 파일 크기가 허용범위 이내인 경우
            ...
            previousImage = newImageUrl; // 현재 선택된 이미지를 이전 이미지로 저장(다음에 바뀔일 대비)
            previousFlie = file; // 현재 선택된 파일 객체를 이전 파일로 저장(다음에 바뀔일에 대비)

        } else{ // 파일 크기가 허용 범위를 초과한 경우
            alert("5MB 이하의 이미지를 선택해주세요!");
            imageInput.value = ""; //1.파일 선택 초기화(alert 창은 띄웠지만 이미 선택된 큰 사이즈 파일을 비우는건 따로)
            // == imageInput.files = null;  << 이렇게 써도 똑같음
            ...
            if(previousFlie) {
                ...
                dataTransfer.items.add(previousFlie);
                // 이전 파일을 추가해두기 : DataTransfer에 File 객체 추가
                imageInput.files = dataTransfer.files;
                // 이전 파일로 input 요소의 files 속성으 복구 : Datatransfer에 저장된
...
deleteImage.addEventListener("click", ()=>{
    ...
    if(profileImg.src !== dafaultImageUrl){
        imageInput.value = ""; // 파일 선택 초기화
        profileImg.src = dafaultImageUrl; // 기본 이미지로 설정
        statusCheck = 0; // 삭제 상태 기록
        previousFlie = null; // 이전 파일 초기화 기록
    } else{
