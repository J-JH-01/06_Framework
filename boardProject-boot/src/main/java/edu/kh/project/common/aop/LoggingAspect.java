...
public class LoggingAspect {
	...
	/** 컨트롤러 수행 전 로그 출력(클래스/메서드/ip(누가보낸거냐))
	 * @param jp
	 */
	@Before("PointcutBundle.controllerPointCut()")
	public void beforeController(JoinPoint jp) {
		
		// AOP가 적용된 클래스 이름 얻어오기 -> 메서드 파라미터에 넣어줘야함
		...
		// ---------------- 요청한 클라이언트 ip 로그 출력 --------------------------
		// 파라미터가 이미 있으면 거기에 HTTPservletRequest로 못불러옴
...
//	     Before/After가 구분되어짐
	...
	public Object aroundServiceImple(ProceedingJoinPoint pjp) throws Throwable{
		...
		// Throwable 주요 자식으로 Exception(예외) / Error(오류)
		
		// 예외 - 개발자가 처리할 수 있는 문제
		// 오류 - 시스템 레벨의 심각한 문제 (개발자가 어떻게 못한다)
