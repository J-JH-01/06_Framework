...
public class ExampleController {
	...
	public String ex1(HttpServletRequest req, Model model) {
		...
		// DTO 객체 Model을 이용해서 html로 전달 (student 클래스 만들것)
		Student std = new Student();
		std.setStudentN("12345");
		std.setName("홍길동");
		std.setAge(22);
		...
		stdList.add(new Student("22222","김이번",22));
		stdList.add(new Student("33333","김삼번",23));
		
		model.addAttribute("stdList",stdList);
		
		return "example/ex1";
		// templates/example/ex1.html 요청 위임
		// 접두사+접미사를 뺀 부분을 리턴시켜줘야함
	}
	...
	@GetMapping("ex4")	//	/example/ex3 GET 방식 요청 매핑
	...
	@GetMapping("ex5")	//	/example/ex3 GET 방식 요청 매핑
