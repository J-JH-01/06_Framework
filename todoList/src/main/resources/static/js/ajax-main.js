...
function getTotalCount(){ // 함수 정의
  ...
  .then (response => { // fetch가 서버에서 응답을 받으면, 
                      //이 응답(매개변수response)을 텍스트 형식으로 변환하는 콜백 함수
  ...
  .then(result =>{  // 첫번째 콜백함수가 완료된 후 호출
    ...
    // == 첫번째 콜백함수에서 반환된 Promise 객체의 Prmomise Result 값
...
// fetch("요청주소").then (res=>{}).then(result=>{console.log(result)})
...
addBtn.addEventListener("click" , () => {
  ...
  .then( result => {
    // 첫번째 then에서 반환된 값 중 변환된 text를 result라는 매개변수에 저장
    if(result > 0) { //성공
      alert("추가 성공!!!");
...
const selectTodoList = () => {
  ...
  .then(todoList => {
    ...
    // text() 하고 출력에 JSON.parse(todoList) 로 출력해도 되긴함
...
changeComplete.addEventListener("click", () => {
  ...
  fetch("/ajax/changeComplete", {
    method: "PUT", // @PutMapping을 사용 가능해진다
    headers : {"Content-Type" : "application/json"},
  ...
  .then( result => {
    ...
    if(result > 0){ // 성공
      ...
      if(complete === 'Y') completeCount.innerText = ++count;
      else                 completeCount.innerText = --count;
...
//js 파일에 함수 호출 코드 바로 작성 -> 페이지 로딩 시 바로 실행하여 화면에 출력하기 위해
getTotalCount(); // 함수 호출
